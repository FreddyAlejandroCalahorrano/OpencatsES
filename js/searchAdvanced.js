...
{
    var html = '<br />Busqueda Avanzada:<br />';
    ...
    for (var i = 0; i < nodes.length; i++)
    {
        ...
        html += '<option value=" AND " '      + ((nodes[i] == " AND "     ) ? 'selected' : '') + '>Y</option>';
        html += '<option value=" OR " '       + ((nodes[i] == " OR "      ) ? 'selected' : '') + '>O</option>';
        html += '<option value=" AND NOT " '  + ((nodes[i] == " AND NOT " ) ? 'selected' : '') + '>NO</option>';
        html += '<option value="* " '         + ((nodes[i] == "* "        ) ? 'selected' : '') + '>Coincidencia</option>';
        html += '<option value=" AND (" '     + ((nodes[i] == " AND ("    ) ? 'selected' : '') + '>Anidada Y</option>';
        html += '<option value=" OR (" '      + ((nodes[i] == " OR ("     ) ? 'selected' : '') + '>Anidada O</option>';
        html += '<option value=" AND NOT (" ' + ((nodes[i] == " AND NOT (") ? 'selected' : '') + '>No Anidada</option>';
    ...
    html += '<br /><br />';
    html += '<input type="button" class="button" id="searchAdvanced" name="searchAdvanced" value="Buscar" onclick="advancedSearchSet(); document.getElementById(\'advancedSearchOn\').value=' + data.length + '; document.getElementById(\'searchForm\').submit();" />&nbsp;';
    html += '<input type="button" class="button" name="simpleSearch" value="Sencilla" onclick="document.getElementById(\'advancedSearchField\').style.display=\'none\';" />&nbsp;';
    html += '<input type="button" class="button" name="resetSearch" value="Reiniciar" onclick="document.getElementById(\'searchText\').value = \'\'; advancedSearchReset();" />&nbsp;';
