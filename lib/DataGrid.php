...
class DataGrid
{
    ...
    public function drawRowsPerPageSelector()
    {
        ...
        foreach (array('15', '30', '50', '100') as $maxResults)
        {
            ...
            {
                echo sprintf(
                    '<option selected="selected" value="%s">%s entradas por página</option>',
            ...
            {
                echo sprintf(
                    '<option value="%s">%s entradas por página</option>',
                    $maxResults, $maxResults
                );
    ...
    {
        echo '<a href="javascript:void(0);" class="button" style="text-decoration: none;" onclick="var filterArea = document.getElementById(\'filterResultsArea', md5($this->_instanceName), '\'); if(filterArea.style.display==\'none\') {filterArea.style.display=\'\'; if (newFilterCounter', md5($this->_instanceName),' == 0){ showNewFilter', md5($this->_instanceName), '();}}else filterArea.style.display=\'none\';">Filtrar</a>';
    }
    ...
    public function drawFilterArea()
    {
        ...
        echo '><legend class="filterAreaLegend">Filtrar</legend>';
        ...
        foreach ($this->_classColumns as $index => $data)
        {
            ...
            if ($filterValue != '')
            {
                ...
                {
                    case '==':
                        $filterOperatorHuman = ' es igual a';
                        break;

                    case '=~':
                        $filterOperatorHuman = ' contiene';
                        break;

                    case '=>':
                        $filterOperatorHuman = ' es mayor que';
                        break;

                    case '=<':
                        $filterOperatorHuman = ' es menos que';
                        break;

                    case '=#':
                        $filterOperatorHuman = ' tiene elemento';
                        break;
                }
                ...
                echo '<img src="images/actions/delete_small.gif" style="padding:0px; margin:0px;" border="0" alt="" title="Quitar este filtro" />';
                echo '</a>&nbsp;';
