...
class SettingsUI extends UserInterface
{
    ...
    public function __construct()
    {
        ...
        $mp = array(
            'Administración' => CATSUtility::getIndexName() . '?m=settings&amp;a=administration',
            'Mi Perfil'     => CATSUtility::getIndexName() . '?m=settings'
        );
        ...
        if (!$fromPostback)
        {
            ...
            if ($questionnaireID != '')
            {
                ...
                if (count($data = $questionnaire->get($questionnaireID)))
                {
                    ...
                    {
                        $questions[$i]['questionTypeLabelquestionTypeLabel'] = $questionnaire->convertQuestionConstantToType(
