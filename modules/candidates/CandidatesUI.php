...
class CandidatesUI extends UserInterface
{
    ...
    public function __construct()
    {
        ...
        $this->_moduleTabText = 'Candidatos';
        $this->_subTabs = array(
            'Agregar candidato'     => CATSUtility::getIndexName() . '?m=candidates&amp;a=add*al=' . ACCESS_LEVEL_EDIT . '@candidates.add',
            'Buscar candidato' => CATSUtility::getIndexName() . '?m=candidates&amp;a=search'
        );
    }
    ...
    private function show()
    {
        ...
        if ($EEOSettingsRS['enabled'] == 1)
        {
            ...
            {
                $EEOValues[] = array('fieldName' => 'Genero', 'fieldValue' => $data['eeoGenderText']);
            }
            if ($EEOSettingsRS['ethnicTracking'] == 1)
            {
                $EEOValues[] = array('fieldName' => 'Etnia', 'fieldValue' => $data['eeoEthnicType']);
            }
            if ($EEOSettingsRS['veteranTracking'] == 1)
            {
                $EEOValues[] = array('fieldName' => 'Estatus de veterano', 'fieldValue' => $data['eeoVeteranType']);
            }
            if ($EEOSettingsRS['disabilityTracking'] == 1)
            {
                $EEOValues[] = array('fieldName' => 'Discapacidad', 'fieldValue' => $data['eeoDisabilityStatus']);
            }
        }
