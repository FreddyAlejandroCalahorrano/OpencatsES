...
<?php if ($this->isJobOrdersMode): ?>
    <?php TemplateUtility::printModalHeader('Job Orders', array('modules/candidates/activityvalidator.js', 'js/activity.js'), 'Órdenes de trabajo: Actividad de Registro'); ?>
<?php elseif ($this->onlyScheduleEvent): ?>
    <?php TemplateUtility::printModalHeader('Candidates', array('modules/candidates/activityvalidator.js', 'js/activity.js'), 'Candidatos: Programar Evento'); ?>
<?php else: ?>
    <?php TemplateUtility::printModalHeader('Candidates', array('modules/candidates/activityvalidator.js', 'js/activity.js'), 'Candidatos: Actividad de Registro'); ?>
<?php endif; ?>
...
<?php endif; ?>
        ...
        <table class="editTable" width="560">
            <tr id="visibleTR" <?php if ($this->onlyScheduleEvent): ?>style="display:none;"<?php endif; ?>>
                <td class="tdVertical">
                    <label id="regardingIDLabel" for="regardingID">Con respecto a:</label>
                </td>
                <td class="tdData">
<?php if ($this->isJobOrdersMode): ?>
...
<?php endif; ?>
            ...
            <tr id="statusTR" <?php if ($this->onlyScheduleEvent): ?>style="display:none;"<?php endif; ?>>
                <td class="tdVertical">
                    <label id="statusIDLabel" for="statusID">Estado:</label>
                </td>
                <td class="tdData">
                    ...
                    <span id="changeStatusSpanA"<?php if ($this->selectedJobOrderID == -1): ?> style="color: #aaaaaa;"<?php endif;?>>Cambiar Estado</span><br />
                    ...
                    <div id="changeStatusDiv" style="margin-top: 4px;">
                        ...
                        <span id="triggerEmailSpan" style="display: none;"><input type="checkbox" name="triggerEmail" id="triggerEmail" onclick="AS_onSendEmailChange('triggerEmail', 'sendEmailCheckTR', 'visibleTR');" />Enviar Notificación por Correo Electrónico al Candidato</span>
                    </div>
                </td>
            </tr>
            ...
            <tr id="sendEmailCheckTR" style="display: none;">
                <td class="tdVertical">
                    <label id="triggerEmailLabel" for="triggerEmail">Correo Electónico:</label>
                </td>
                <td class="tdData">
                    Mensaje Personalizado<br />
           ...
           <tr id="addActivityTR" <?php if ($this->onlyScheduleEvent): ?>style="display:none;"<?php endif; ?>>
                ...
                <td class="tdData">
                    <input type="checkbox" name="addActivity" id="addActivity" style="margin-left: 0px;"<?php if (!$this->onlyScheduleEvent): ?> checked="checked"<?php endif; ?> onclick="AS_onAddActivityChange('addActivity', 'activityTypeID', 'activityNote', 'addActivitySpanA', 'addActivitySpanB');" />Registrar una Actividad<br />
                    <div id="activityNoteDiv" style="margin-top: 4px;">
                        ...
                        <select id="activityTypeID" name="activityTypeID" class="inputbox" style="width: 150px; margin-bottom: 4px;">
                            ...
                            <option value="<?php echo(ACTIVITY_MEETING); ?>">Entrevista</option>
                            <option value="<?php echo(ACTIVITY_OTHER); ?>">Otra</option>
                        </select><br />
                        <span id="addActivitySpanB">Notas de Actividad</span><br />
            ...
            <tr id="scheduleEventTR">
                ...
                <td class="tdData">
                    <input type="checkbox" name="scheduleEvent" id="scheduleEvent" style="margin-left: 0px; <?php if ($this->onlyScheduleEvent): ?>display:none;<?php endif; ?>" onclick="AS_onScheduleEventChange('scheduleEvent', 'scheduleEventDiv');"<?php if ($this->onlyScheduleEvent): ?> checked="checked"<?php endif; ?> /><?php if (!$this->onlyScheduleEvent): ?>Programar Evento<?php endif; ?>
                        ...
                        <table style="border: none; margin: 0px; padding: 0px;">
                            <tr>
                                <td valign="top">
                                    ...
                                    <div style="margin-bottom: 4px;">
                                        <input type="radio" name="allDay" id="allDay1" value="1" style="margin-left: 0px" onchange="AS_onEventAllDayChange('allDay1');" />Todo el Día /Sin Tiempo Específico<br />
                                    </div>
                                ...
                                <td valign="top">
                                    ...
                                    <div style="display:none;" id="reminderArea">
                                        <div>
                                            <label>Correo Electrónico a:</label><br />
...
<?php else: ?>
    ...
    <?php else: ?>
         <?php if (!$this->onlyScheduleEvent): ?>
            ...
            <?php if ($this->statusChanged): ?>
                <p>El estado del candidato ha cambiado de <span class="bold"><?php $this->_($this->oldStatusDescription); ?></span> a <span class="bold"><?php $this->_($this->newStatusDescription); ?></span>.</p>
            <?php else: ?>
                <p>El estado del candidato no ha cambiado.</p>
            <?php endif; ?>
            ...
            <?php if ($this->activityAdded): ?>
                ...
                <?php else: ?>
                    <p>Una Entrada de Actividad de tipo <span class="bold"><?php $this->_($this->activityType); ?></span> se ha añadido sin notas.</p>
                <?php endif; ?>
            <?php else: ?>
    ...
    <form>
<?php if ($this->isJobOrdersMode): ?>
        <input type="button" name="close" class="button" value="Cerrar" onclick="parentGoToURL('<?php echo(CATSUtility::getIndexName()); ?>?m=joborders&amp;a=show&amp;jobOrderID=<?php echo($this->regardingID); ?>');" />
<?php else: ?>
        <input type="button" name="close" class="button" value="Cerrar" onclick="parentGoToURL('<?php echo(CATSUtility::getIndexName()); ?>?m=candidates&amp;a=show&amp;candidateID=<?php echo($this->candidateID); ?>');" />
<?php endif; ?>
    </form>
<?php endif; ?>
