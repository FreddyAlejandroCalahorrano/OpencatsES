...
{
    $interface->outputXMLErrorPage(-1, 'No hay listas para agregar aprobadas.');
    die();
}

if (!isset($_REQUEST['itemsToAdd']))
{
    $interface->outputXMLErrorPage(-1, 'No se aprobaron elementos para agregar.');
    die();
}

if (!$interface->isRequiredIDValid('dataItemType'))
{
    $interface->outputXMLErrorPage(-1, 'Tipo de lista guardada no válido.');
    die();
}
...
foreach ($listsToAdd as $index => $data)
{
    ...
    else
    {
        ...
        {
            $interface->outputXMLErrorPage(-1, 'Valor de listas no válido. ('.$data.')');
            die;
        }
    }
}
...
foreach ($itemsToAdd as $index => $data)
{
    ...
    else
    {
        if (isRequiredValueValid($data) == false)
        {
            $interface->outputXMLErrorPage(-1, 'Valor de artículos no válido.');
            die;
        }
    }
}
